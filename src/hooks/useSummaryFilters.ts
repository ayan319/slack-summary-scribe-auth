...
export const useSummaryFilters = (summaries: HistoryItem[], filters: FilterOptions) => {
  return useMemo(() => {
    ...
    if (filters.tags.length > 0) {
      filtered = filtered.filter(item =>
        filters.tags.some(filterTag => 
          item.summary.tags?.includes(filterTag) || item.tags?.includes(filterTag)
        )
      );
    }
    ...
    if (filters.rating !== null) {
      filtered = filtered.filter(item => {
        const rating = item.summary.userRating || item.userRating || item.summary.rating;
